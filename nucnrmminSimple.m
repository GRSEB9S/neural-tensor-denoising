function [X,D] = nucnrmminSimple( Y, opts )
% x = nucnrmmin( Y, opts )
...
nz = logical(sum(Y,2));
Y = Y(nz,:); % remove rows with no spikes
[N,T] = size(Y);
    ...
    while stopping/norm(x,'fro') > 1e-6 % Outer loop of Newton's method
        switch nlin
            case 'exp'
                ...
                g =  exp( x ) - Y; % gradient
            otherwise
                h =  d2f( x ) - Y .* ( d2f( x ) .* f( x ) - df( x ).^2 ) ./ f( x ).^2;
                g =  df ( x ) - Y .* df( x ) ./ f( x );
                g(isnan(g)) = 0;
                h(isnan(h)) = 0;
        end
        
        grad = g + rho*x - rho*X - Z;
...
X = zeros(length(nz),T);
X( nz,:) = x;
X(~nz,:) = -Inf; % set firing rates to zero for rows with no data. Used to make sure the returned value is aligned with the input
