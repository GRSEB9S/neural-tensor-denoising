function [X,Zm,Am] = tensorADMM(Y,xnat)
   %[ X,Zm,Am ] = tensorADMMfun( Y,xnat ) finds underlying rate X for spiking
...
%% ADMM
   ...
   eta = 1/(10*nanstd(Y(:))); % admm parameter, choose factor in front of std(y), see Tomioka (2010) // how to choose? mean(y) instead of std(y) since poisson?
   beta = 100; % total variation parameter // how to choose?
   gamma = 1*ones(3,1); % sum of nuclear norms parameter // how to choose?
   ...
   Z = 1 + 0.02*randn(sz); % ADMM aux variable // how to initialize?
   A = 1 + 0.02*randn(sz); % lagrange mult // how to initialize?
   ...
   f   = @(x) log(1+exp(x));
   df  = @(x) 1./(1+exp(-x)); % derivative 
   d2f = @(x) exp(-x)./(1+exp(-x)).^2; % 2nd derivative
      ...
      switch method
         case 1
            ...
            while stopping/norm(X(:),'fro') > 1e-6
               ...
               % total var term
               dia = 2*ones(t,1); dia(1) = 1; dia(end) = 1; % make a block
               off = -ones(t-1,1);
         ...
         case 2
            %% method 2 - do newtons method for each individual tx1 vector (not yet implemented), probably faster
